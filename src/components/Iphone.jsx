...
export function Model(props) {
  ...
  useEffect(() => {
    const validHex = props.item.color[0];
    const isValidHex = /^#[0-9A-Fa-f]{6}$/.test(validHex);
    const hex_code = isValidHex ? validHex : "#8F8A88";
    Object.entries(materials).map((material) => {
      if (
        ...
        material[1].color = new THREE.Color(hex_code);
      }
